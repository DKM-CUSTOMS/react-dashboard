...
/**
 * Modifies fields on an existing outbound record and recalculates saldo
 * @param {string} inboundMrn - The parent MRN
 * @param {string} outboundMrn - The child outbound MRN to modify
 * @param {Object} updatedFields - Object with field names and new values
 */
export async function modifyOutbound(inboundMrn, outboundMrn, updatedFields) {
  const API_URL = "https://functionapp-python-uploads-huaafaf5f0cxc8g4.westeurope-01.azurewebsites.net/api/DgMRNExtractor";
  const API_KEY = import.meta.env.VITE_API_V2_KEY;
  
  const payload = {
    route: "modify_outbound",
    inbound_mrn: inboundMrn,
    outbound_mrn: outboundMrn,
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`Failed to modify outbound: ${response.status} ${response.statusText} - ${errorText}`);
    }
    ...
    console.error('Error modifying outbound:', error);
    throw error;
  }
}
